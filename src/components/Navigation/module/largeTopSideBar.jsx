import PropTypes from 'prop-types'
import { useState, useContext} from 'react';
import { useNavigate, Link } from 'react-router-dom';

import { RxHamburgerMenu } from 'react-icons/rx';
import { TfiSearch } from 'react-icons/tfi';
import { RiUserVoiceLine, RiNotification3Line } from 'react-icons/ri';
import { BiMessageSquareDetail } from 'react-icons/bi';
import { SearchComponent } from '../../modules/Search/SearchComponent';
import Logo from '../../../assets/logo.png';
import '../style/topSideBar/topSideBar.css';

import { ResolutionContext } from '../../../context/resolution';
import { MobileSearchComponent } from '../../modules/Search/MobileSearchComponent';

export const LargeTopSideBarNavigation = ({ toggleLeftSideMenu }) => {

    const [searchMovie, setSearchMovie] = useState('');
    const resolution = useContext(ResolutionContext);
    const [isMenuOpen, setIsMenuOpen] = useState(false);

    const navigate = useNavigate();
 
    const onUserSearch = (e) => {
        setSearchMovie(e.target.value);
    };
    const onUserPressEnter = (e) => {
        const key = e.key || e.keyCode;

        if(key !== 'Enter')
            return;

        if(!searchMovie)
            return console.log('searchMovie is empty');

        navigate(`/search?query=${encodeURIComponent(searchMovie)}`);
    };

    const handleMobileSearch = () => {
        if(resolution !== 'mobile')
            return;

        setIsMenuOpen(!isMenuOpen);
    };

    return (
        <div className={`largeTopNavbar flex items-center justify-between h-28`}>
            <div className="topLeftSideContent w-full flex items-center px-2">
                
                <div className="leftSideNavbarLogo flex justify-center md:w-64">
                    <Link to={'/'}>
                        <img 
                            className='border-[1px] w-12 p-2 rounded-full border-red-600'
                            src={Logo} 
                            alt="MovieMasterLogo" />
                    </Link>
                </div>

                <div className="leftSideButtons w-full flex ml-3 gap-5 items-center">
                    <div className="manageLeftBarMobile">
                        <button
                            onClick={toggleLeftSideMenu}    
                        >
                            <RxHamburgerMenu />
                        </button>
                    </div>
                    
                    {
                        resolution === 'mobile' ? (
                            <div className="manageLeftBarMobile">
                                <button
                                    onClick={handleMobileSearch}    
                                >
                                    <TfiSearch />
                                </button>
                            </div>
                        ) : (
                        <div className="searchFieldInput w-full flex items-center p-2 bg-black/5 border-2 rounded-2xl lg:w-2/5">
                            <div className="searchInput w-full flex flex-row-reverse items-center">
                                <SearchComponent 
                                    className={'bg-transparent w-full focus:outline-none px-1'}
                                    searchInput={onUserSearch}
                                    onPressEnter={onUserPressEnter}
                                />
                                <span className='text-sm p-2'><TfiSearch /></span>
                            </div>

                            <div className="searchButton flex items-center">
                                <button 
                                    className='p-2'>
                                    <RiUserVoiceLine />
                                </button>
                            </div>
                        </div>
                        )
                    }
                    { isMenuOpen && <MobileSearchComponent /> }
                </div>


            </div>

            <div className="topRightSideContent w-auto flex items-center justify-end mx-5 gap-5">
                
                <div className="quickAccesButtons flex gap-5">
                    <div className="profileNotifications">
                        <button className='text-[1.25rem]'>
                            <RiNotification3Line />
                        </button>
                    </div>
                    <div className="profileMessages">
                        <button className='text-[1.25rem]'>
                            <BiMessageSquareDetail />
                        </button>
                    </div>
                </div>

                <div className="profileSection flex items-center bg-white/25 p-2 rounded-lg cursor-pointer">
                    <div className="profileIcon mx-2">
                        <img 
                        className='w-[9rem] lg:w-20 rounded-full'
                        src="data:image/png;base64," 
                        alt="google-icon" />
                    </div>

                    {
                        resolution !== 'mobile' &&
                        <div className="profileInformations flex flex-col">
                            <span className='w-20'>Maxim D</span>
                            <span className='text-gray-400 text-[11px]'>watched 13h</span>
                        </div>
                    }
</div>

            </div>
        </div>
    )
};

LargeTopSideBarNavigation.propTypes = {
    toggleLeftSideMenu: PropTypes.func
};
